...
describe('purgeExpiredItems', () => {
  ...
  it('보관 기간 만료 항목 영구 삭제', async () => {
    ...
    const purged = await purgeExpiredItems(30);
  ...
  it('보관 기간 내 항목은 유지', async () => {
    ...
    expect(await getDeletedQuestions()).toHaveLength(1);
  });
  ...
  it('JD 만료 시 영구 삭제', async () => {
    ...
    expect(await getDeletedJobs()).toHaveLength(0);
  });
  ...
  it('retentionDays 미지정 시 DEFAULT_RETENTION_DAYS 사용', async () => {
    ...
    const purged = await purgeExpiredItems(); // default = 30
    expect(purged.questions).toBe(1);
  });
});
