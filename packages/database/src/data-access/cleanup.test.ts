...
import { interviewQuestions, followupQuestions, jobDescriptions } from '../schema';
import {
  createTestDb,
  cleanupTestDb,
  seedTestQuestions,
...
describe('purgeAllExpiredItems', () => {
  ...
  it('보관 기간 만료 항목 영구 삭제', async () => {
    ...
    await softDeleteQuestion(db, DEFAULT_USER_ID, qId);
    ...
    const purged = await purgeAllExpiredItems(db, 30);
    expect(purged.questions).toBe(1);
    expect(await getDeletedQuestions(db, DEFAULT_USER_ID)).toHaveLength(0);
  });
  ...
  it('보관 기간 내 항목은 유지', async () => {
    ...
    expect(await getDeletedQuestions(db, DEFAULT_USER_ID)).toHaveLength(1);
  });
  ...
  it('JD 만료 시 영구 삭제', async () => {
    ...
    await softDeleteJob(db, DEFAULT_USER_ID, jdId);
    await db
    ...
    expect(await getDeletedJobs(db, DEFAULT_USER_ID)).toHaveLength(0);
  });
  ...
  it('만료된 followup도 영구 삭제', async () => {
    ...
    const questions = await getLibraryQuestions(db, DEFAULT_USER_ID);
    const qId = questions[0].id;
    const followups = await getFollowupsByQuestionId(db, DEFAULT_USER_ID, qId);
    const fId = followups[0].id;

    await softDeleteFollowup(db, DEFAULT_USER_ID, fId);
    await db
  ...
  it('retentionDays 미지정 시 DEFAULT_RETENTION_DAYS 사용', async () => {
    ...
    const purged = await purgeAllExpiredItems(db); // default = 30
    expect(purged.questions).toBe(1);
  });
});
