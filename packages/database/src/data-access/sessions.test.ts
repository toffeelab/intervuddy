...
describe('sessions data-access', () => {
  ...
  describe('createSession', () => {
    it('세션을 생성하고 id를 반환한다', async () => {
      const id = await createSession(db, DEFAULT_USER_ID, {
        title: '모의면접 세션',
        role: 'interviewer',
      });
    ...
    it('생성자를 참가자로 자동 추가한다', async () => {
      ...
      const session = await getSessionById(db, DEFAULT_USER_ID, sessionId);
  ...
  describe('getSessionById', () => {
    it('참가자만 세션을 조회할 수 있다', async () => {
      ...
      const otherView = await getSessionById(db, OTHER_USER_ID, sessionId);
      expect(otherView).toBeNull();
    });

    it('존재하지 않는 세션은 null을 반환한다', async () => {
      const session = await getSessionById(db, DEFAULT_USER_ID, 'nonexistent-id');
  ...
  describe('getSessionsByUserId', () => {
    it('빈 DB에서 빈 배열을 반환한다', async () => {
      const sessions = await getSessionsByUserId(db, DEFAULT_USER_ID);
      expect(sessions).toEqual([]);
    });

    it('참가 중인 세션 목록을 createdAt 내림차순으로 반환한다', async () => {
      await createSession(db, DEFAULT_USER_ID, {
        title: '세션 1',
        role: 'interviewer',
      });
      await createSession(db, DEFAULT_USER_ID, {
    ...
    it('다른 유저의 세션은 포함하지 않는다', async () => {
      ...
      await createSession(db, OTHER_USER_ID, {
        title: '다른 세션',
        role: 'interviewer',
      });

      const mySessions = await getSessionsByUserId(db, DEFAULT_USER_ID);
  ...
  describe('updateSessionStatus', () => {
    it('in_progress로 변경 시 startedAt을 설정한다', async () => {
      ...
      await updateSessionStatus(db, DEFAULT_USER_ID, sessionId, 'in_progress');
      const session = await getSessionById(db, DEFAULT_USER_ID, sessionId);
    ...
    it('completed로 변경 시 endedAt을 설정한다', async () => {
      ...
      await updateSessionStatus(db, DEFAULT_USER_ID, sessionId, 'in_progress');
      await updateSessionStatus(db, DEFAULT_USER_ID, sessionId, 'completed');
      const session = await getSessionById(db, DEFAULT_USER_ID, sessionId);
  ...
  describe('deleteSession', () => {
    it('생성자가 소프트 삭제할 수 있다', async () => {
      ...
      await deleteSession(db, DEFAULT_USER_ID, sessionId);
      const session = await getSessionById(db, DEFAULT_USER_ID, sessionId);
      expect(session).toBeNull();
    });
    ...
    it('생성자가 아닌 유저는 삭제할 수 없다', async () => {
      ...
      await deleteSession(db, OTHER_USER_ID, sessionId);
      const session = await getSessionById(db, DEFAULT_USER_ID, sessionId);
      expect(session).not.toBeNull();
    });
