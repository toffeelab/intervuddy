...
describe('session-records data-access', () => {
  ...
  describe('recordQuestion', () => {
    it('질문을 기록하고 id를 반환한다', async () => {
      ...
      const id = await recordQuestion(db, sessionId, {
        content: '자기소개를 해주세요',
        displayOrder: 1,
      });
  ...
  describe('recordAnswer', () => {
    it('답변을 기록하고 id를 반환한다', async () => {
      ...
      const questionId = await recordQuestion(db, sessionId, {
        content: '질문',
        displayOrder: 1,
      });

      const answerId = await recordAnswer(db, questionId, DEFAULT_USER_ID, '답변 내용입니다');
  ...
  describe('recordFeedback', () => {
    it('피드백을 기록하고 id를 반환한다', async () => {
      ...
      const feedbackId = await recordFeedback(db, questionId, OTHER_USER_ID, '좋은 답변입니다', 8);
    ...
    it('content나 score가 null일 수 있다', async () => {
      ...
      const feedbackId = await recordFeedback(db, questionId, OTHER_USER_ID, null, null);
      expect(feedbackId).toBeDefined();
    });
  });
  ...
  describe('getSessionRecords', () => {
    it('질문 + 답변 + 피드백을 포함한 전체 기록을 반환한다', async () => {
      ...
      const q1Id = await recordQuestion(db, sessionId, {
        content: '첫 번째 질문',
        displayOrder: 1,
      });
      const q2Id = await recordQuestion(db, sessionId, {
        content: '두 번째 질문',
        displayOrder: 2,
      });

      await recordAnswer(db, q1Id, DEFAULT_USER_ID, '첫 번째 답변');
      await recordFeedback(db, q1Id, OTHER_USER_ID, '잘했습니다', 9);

      const records = await getSessionRecords(db, sessionId);
  ...
  describe('getSessionQuestionByDisplayOrder', () => {
    it('displayOrder로 질문을 찾는다', async () => {
      ...
      const result = await getSessionQuestionByDisplayOrder(db, sessionId, 1);
    ...
    it('존재하지 않는 displayOrder는 null을 반환한다', async () => {
      ...
      const result = await getSessionQuestionByDisplayOrder(db, sessionId, 99);
