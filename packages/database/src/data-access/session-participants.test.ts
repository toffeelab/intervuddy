...
describe('session-participants data-access', () => {
  ...
  describe('addParticipant', () => {
    ...
    it('생성자가 아닌 유저는 참가자를 추가할 수 없다', async () => {
      ...
      await expect(
        addParticipant(db, OTHER_USER_ID, sessionId, THIRD_USER_ID, 'reviewer')
      ).rejects.toThrow();
    });
    ...
    it('interviewer는 최대 1명까지만 가능하다', async () => {
      ...
      await expect(
        addParticipant(db, DEFAULT_USER_ID, sessionId, OTHER_USER_ID, 'interviewer')
      ).rejects.toThrow();
    });
    ...
    it('interviewee는 최대 1명까지만 가능하다', async () => {
      ...
      await expect(
        addParticipant(db, DEFAULT_USER_ID, sessionId, OTHER_USER_ID, 'interviewee')
      ).rejects.toThrow();
    });
    ...
    it('같은 유저를 중복 추가할 수 없다', async () => {
      ...
      await addParticipant(db, DEFAULT_USER_ID, sessionId, OTHER_USER_ID, 'reviewer');
      await expect(
        addParticipant(db, DEFAULT_USER_ID, sessionId, OTHER_USER_ID, 'reviewer')
      ).rejects.toThrow();
    });
  });
  ...
  describe('removeParticipant', () => {
    ...
    it('생성자가 아닌 유저는 참가자를 제거할 수 없다', async () => {
      ...
      await addParticipant(db, DEFAULT_USER_ID, sessionId, OTHER_USER_ID, 'reviewer');
      await expect(
        removeParticipant(db, OTHER_USER_ID, sessionId, THIRD_USER_ID)
      ).rejects.toThrow();
    });
  });
