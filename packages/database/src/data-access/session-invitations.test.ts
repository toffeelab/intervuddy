...
describe('session-invitations data-access', () => {
  ...
  describe('createInvitation', () => {
    it('초대를 생성하고 id와 inviteCode를 반환한다', async () => {
      ...
      const result = await createInvitation(db, DEFAULT_USER_ID, sessionId, 'interviewee');
    ...
    it('기본 만료시간은 24시간이다', async () => {
      ...
      const result = await createInvitation(db, DEFAULT_USER_ID, sessionId, 'interviewee');
      const invitation = await getInvitationByCode(db, result.inviteCode);
  ...
  describe('getInvitationByCode', () => {
    it('초대 코드로 초대 정보를 조회한다', async () => {
      ...
      const { inviteCode } = await createInvitation(db, DEFAULT_USER_ID, sessionId, 'interviewee');
      const invitation = await getInvitationByCode(db, inviteCode);
      expect(invitation).not.toBeNull();
    ...
    it('존재하지 않는 코드는 null을 반환한다', async () => {
      const invitation = await getInvitationByCode(db, 'nonexist');
  ...
  describe('acceptInvitation', () => {
    it('초대를 수락하고 참가자로 추가된다', async () => {
      ...
      const { inviteCode } = await createInvitation(db, DEFAULT_USER_ID, sessionId, 'interviewee');
      const result = await acceptInvitation(db, inviteCode, OTHER_USER_ID);
    ...
    it('만료된 초대는 수락할 수 없다', async () => {
      ...
      await expect(acceptInvitation(db, inviteCode, OTHER_USER_ID)).rejects.toThrow();
    });
    ...
    it('최대 사용 횟수를 초과하면 수락할 수 없다', async () => {
      ...
      const { inviteCode } = await createInvitation(db, DEFAULT_USER_ID, sessionId, 'reviewer', {
        maxUses: 1,
      });

      await acceptInvitation(db, inviteCode, OTHER_USER_ID);
      await expect(acceptInvitation(db, inviteCode, THIRD_USER_ID)).rejects.toThrow();
    });
    ...
    it('취소된 초대는 수락할 수 없다', async () => {
      ...
      const { id, inviteCode } = await createInvitation(
        db,
        DEFAULT_USER_ID,
        sessionId,
        'interviewee'
      );
      await revokeInvitation(db, DEFAULT_USER_ID, id);
  ...
  describe('revokeInvitation', () => {
    ...
    it('초대자가 아닌 유저는 취소할 수 없다', async () => {
      ...
      const { id } = await createInvitation(db, DEFAULT_USER_ID, sessionId, 'interviewee');
      await expect(revokeInvitation(db, OTHER_USER_ID, id)).rejects.toThrow();
    });
  });
});
