...
describe('importQuestionsToJob', () => {
  ...
  it('라이브러리 질문을 JD로 복사 (질문 + 키워드 + 꼬리질문)', async () => {
    ...
    const result = await importQuestionsToJob(db, DEFAULT_USER_ID, {
      jdId: jobs[0].id,
    ...
    const jdQuestions = await getQuestionsByJdId(db, DEFAULT_USER_ID, jobs[0].id);
  ...
  it('이미 가져온 질문은 스킵', async () => {
    ...
    await importQuestionsToJob(db, DEFAULT_USER_ID, {
      jdId: jobs[0].id,
      questionIds: [questions[0].id],
    });
    const result = await importQuestionsToJob(db, DEFAULT_USER_ID, {
