...
describe('questions data-access', () => {
  ...
  describe('getLibraryQuestions', () => {
    it('빈 DB에서 빈 배열을 반환한다', async () => {
      expect(await getLibraryQuestions(db, DEFAULT_USER_ID)).toEqual([]);
    });
    ...
    it('삭제된 질문은 제외한다', async () => {
      await seedTestQuestions(db);
      const questions = await getLibraryQuestions(db, DEFAULT_USER_ID);
      await softDeleteQuestion(db, DEFAULT_USER_ID, questions[0].id);
      expect(await getLibraryQuestions(db, DEFAULT_USER_ID)).toEqual([]);
    });
  });
  ...
  describe('getQuestionsByJdId', () => {
    it('JD에 속한 질문을 반환한다', async () => {
      ...
      const questions = await getQuestionsByJdId(db, DEFAULT_USER_ID, jobs[0].id);
      expect(questions).toHaveLength(1);
  ...
  describe('getQuestionsByCategory', () => {
    it('카테고리별 질문을 반환한다', async () => {
      await seedTestQuestions(db);
      const cats = await db.select({ id: interviewCategories.id }).from(interviewCategories);
      const questions = await getQuestionsByCategory(db, DEFAULT_USER_ID, cats[0].id);
      expect(questions).toHaveLength(1);
    ...
    it('다른 카테고리의 질문은 포함하지 않는다', async () => {
      ...
      const questions = await getQuestionsByCategory(db, DEFAULT_USER_ID, cats[1].id);
  ...
  describe('createQuestion', () => {
    it('질문을 생성하고 id를 반환한다', async () => {
      await seedTestCategories(db);
      const cats = await db.select({ id: interviewCategories.id }).from(interviewCategories);
      const id = await createQuestion(db, DEFAULT_USER_ID, {
        categoryId: cats[0].id,
        question: '새 질문',
        answer: '새 답변',
        tip: '팁',
  ...
  describe('updateQuestion', () => {
    it('질문을 부분 수정할 수 있다', async () => {
      await seedTestQuestions(db);
      const questions = await getLibraryQuestions(db, DEFAULT_USER_ID);
      await updateQuestion(db, DEFAULT_USER_ID, { id: questions[0].id, answer: '수정된 답변' });
      const updated = await getLibraryQuestions(db, DEFAULT_USER_ID);
  ...
  describe('updateQuestion - categoryId 변경 불가', () => {
    it('categoryId를 전달해도 무시된다 (display_order 보호)', async () => {
      ...
      await updateQuestion(db, DEFAULT_USER_ID, { id: questions[0].id, categoryId: cat2.id });

      // categoryId는 변경되지 않음
      expect(await getQuestionsByCategory(db, DEFAULT_USER_ID, cat1.id)).toHaveLength(1);
      expect(await getQuestionsByCategory(db, DEFAULT_USER_ID, cat2.id)).toHaveLength(0);
    });
    ...
    it('categoryId 무시 시에도 다른 필드는 정상 수정된다', async () => {
      ...
      const [cat1, cat2] = cats;
      const questions = await getLibraryQuestions(db, DEFAULT_USER_ID);
      await updateQuestion(db, DEFAULT_USER_ID, {
        id: questions[0].id,
        categoryId: cat2.id,
        answer: '수정된 답변',
      });
      const updated = await getQuestionsByCategory(db, DEFAULT_USER_ID, cat1.id);
  ...
  describe('updateQuestionKeywords', () => {
    it('기존 키워드를 새 키워드 목록으로 교체한다', async () => {
      await seedTestQuestions(db);
      const questions = await getLibraryQuestions(db, DEFAULT_USER_ID);
      await updateQuestionKeywords(db, DEFAULT_USER_ID, questions[0].id, [
        '소통',
        '리더십',
        '성장',
      ]);
    ...
    it('키워드 변경 결과가 getLibraryQuestions()에 반영된다', async () => {
      await seedTestQuestions(db);
      const questions = await getLibraryQuestions(db, DEFAULT_USER_ID);
      await updateQuestionKeywords(db, DEFAULT_USER_ID, questions[0].id, ['신규키워드']);
      const updated = await getLibraryQuestions(db, DEFAULT_USER_ID);
  ...
  describe('softDeleteQuestion / restoreQuestion', () => {
    it('소프트 삭제 후 복원할 수 있다', async () => {
      ...
      await restoreQuestion(db, DEFAULT_USER_ID, questions[0].id);
      expect(await getLibraryQuestions(db, DEFAULT_USER_ID)).toHaveLength(1);
    });
  });
  ...
  describe('getDeletedQuestions', () => {
    it('삭제된 질문을 반환한다', async () => {
      ...
      const questions = await getLibraryQuestions(db, DEFAULT_USER_ID);
      await softDeleteQuestion(db, DEFAULT_USER_ID, questions[0].id);
      const deleted = await getDeletedQuestions(db, DEFAULT_USER_ID);
    ...
    it('jdId로 필터링할 수 있다', async () => {
      ...
      await softDeleteQuestion(db, DEFAULT_USER_ID, libQ.id);
      await softDeleteQuestion(db, DEFAULT_USER_ID, jdQ.id);
      const deleted = await getDeletedQuestions(db, DEFAULT_USER_ID, jobs[0].id);
      expect(deleted).toHaveLength(1);
