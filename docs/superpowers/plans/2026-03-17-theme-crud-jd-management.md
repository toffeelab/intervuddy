...
import { getAllJobs, createJob, updateJob, updateJobStatus, softDeleteJob, restoreJob } from './jobs';
...
import { describe, it, expect, beforeEach, afterEach } from 'vitest';
import Database from 'better-sqlite3';
import { createTestDb, cleanupTestDb, seedTestQuestions, seedTestJobDescription } from '@/test/helpers/db';
...
describe('purgeExpiredItems', () => {
  it('보관 기간 만료 항목 영구 삭제', () => {
    ...
    db.prepare("UPDATE interview_questions SET deleted_at = datetime('now', '-31 days') WHERE id = 1").run();
  ...
  it('보관 기간 내 항목은 유지', () => {
    ...
    db.prepare("UPDATE interview_questions SET deleted_at = datetime('now', '-29 days') WHERE id = 1").run();
